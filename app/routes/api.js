...
users.findOne({_id:req.headers['userid']},function(err,data){
  if(data===null || data.length===0){
    commonService.error(res,"BAD_REQUEST",500);
      ...
      if(data.status==22 && data.type=='phone'){
        ...
        resHandler.created(res,{"userStatusCode":data.status});
      }
    }
});
}
...
router.post('/', function (req, res,next) {
  console.log(req.body);
    if(req.originalUrl=='/api/users/addUser' || req.originalUrl=='/api/oauth/token' ||req.originalUrl=='/api/users/forgetPassword' ||req.originalUrl=='/api/users/resetpassword'|| req.originalUrl=='/api/users/otpVerification'){
      next();
    }
    else{
    users.findOne({_id:req.headers['userid']},function(err,data){
